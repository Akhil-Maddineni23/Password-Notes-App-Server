...
const SharedNotesModel  = require('./src/models/SharedNotes');
...
// Listen for 'expire' events in the SharedNotes collection
SharedNotesModel.watch([deleteOps]).on('change', async (change) => {
 
    const sharedNoteId = change.documentKey._id;
    try {
      console.log("Entereed ----");
      // Remove the reference from the User schema's receivedNotes array
      await UserModel.updateMany({}, { $pull: { receivedNotes: sharedNoteId } });
    } catch (error) {
      console.error('Error removing sharedNoteId from receivedNotes:', error);
    }
